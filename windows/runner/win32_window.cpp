...
// Define un atributo de ventana que habilita las decoraciones de ventana en modo oscuro.
...
// Contador de instancias de Win32Window.
static int g_active_window_count = 0;

// Función para escalar valores lógicos a físicos.
int Scale(int source, double scale_factor) {
  return static_cast<int>(source * scale_factor);
}

// Habilita el soporte de DPI completo si está disponible.
...
bool Win32Window::Create(const std::wstring& title, const Point& origin, const Size& size) {
  ...
  if (!window) {
    return false;  // Manejo de errores: no se pudo crear la ventana
  }
...
// Establece el procedimiento de ventana.
...
bool Win32Window::OnCreate() {
  return true;  // No-op; proporcionado para subclases.
}

void Win32Window::OnDestroy() {
  // No-op; proporcionado para subclases.
}
